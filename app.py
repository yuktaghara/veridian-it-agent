...
user_query = st.text_area("How can IT help you today?", placeholder="e.g., I'm locked out of my account after 6 attempts...")
...
def run_agent_logic(query, email):
    ...
    if "phishing" in q or "forwarding" in q or "suspicious" in q:
        action = "CRITICAL BLOCK: Stop forwarding email immediately. Alerting security@veridian-corp.example."
        source = "KB-09: Security Incident Reporting"
        status = "Escalated to Security"
    elif "guest" in q and ("wifi" in q or "wi-fi" in q):
        action = "RESOLVED: Generate 24-hour guest Wi-Fi credentials directly at the front-desk kiosk."
        source = "KB-07: Guest Wi-Fi Access"
        status = "Resolved (Self-Service)"
    elif "locked out" in q or "password" in q or "attempts" in q:
        action = "ACTION QUEUED: Account locked (>5 attempts). Manual unlock ticket created for IT."
        ...
        action = "ELIGIBLE: Laptop meets 3-year threshold (or verified hardware failure). Replacement ticket logged."
        source = "KB-03 & Asset Management Policy"
        status = "Pending Fulfillment"
    elif len(query.strip().split()) < 4:
        action = "CLARIFICATION NEEDED: Please provide specific details about the device, app, or error message."
        source = "Agent Ambiguity Guardrail"
        status = "Waiting on Employee Response"
    else:
        action = "LOGGED: Ticket received and routed to standard IT support queue."
...
if st.button("Submit Request"):
    if user_query:
        ...
        st.write(f"**Action/Resolution:** {res['Agent Decision & Next Steps']}")
